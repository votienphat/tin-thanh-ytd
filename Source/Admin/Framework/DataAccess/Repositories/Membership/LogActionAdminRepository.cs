...
namespace DataAccess.Repositories.Membership
{
    public class LogActionAdminRepository : DaoRepository<MetroMembershipEntities, LogActionAdmin>, ILogActionAdminRepository
    {
        ...
        public List<Ins_LogActionAdmin_GetLockedLogByTime_Result> GetLockedLogByTime(DateTime fromDateTime, DateTime toDateTime, int actionId)
        {
            return Uow.Context.Ins_LogActionAdmin_GetLockedLogByTime(fromDateTime, toDateTime, actionId).ToList();
        }
